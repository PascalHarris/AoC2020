	...
	@autoreleasepool {
		...
		for (int i = 0; i < reportArray.count; i++) {
			...
			for (int j = 0; j < reportArray.count; j++) {
				...
				if ((number2 > target_number - number1) || (solution > 0)) {
					break;
				}
				for (int k = 0; k < reportArray.count; k++) {
					int number3 = [reportArray[k] intValue];
					if (number1 + number2 + number3 == target_number) {
						solution = number1 * number2 * number3;
						break;
					} else if (number3 > target_number - (number1 + number2)) {
						break;
					}
				}
			}
